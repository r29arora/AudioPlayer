...
public class AudioPlayer: NSObject {
    ...
    public override init() {
        ...
        observe(ReachabilityChangedNotification, selector: #selector(AudioPlayer.reachabilityStatusChanged), object: reachability)
    ...
    private var player: AVPlayer? {
        didSet {
            ...
            if let player = player {
                ...
                let timer = NSTimer(timeInterval: adjustQualityTimeInternal, target: target, selector: #selector(ClosureContainer.callSelectorOnTarget), userInfo: nil, repeats: false)
                ...
                #if os(iOS) || os(tvOS)
                    observe(AVAudioSessionInterruptionNotification, selector: #selector(AudioPlayer.audioSessionGotInterrupted))
                    observe(AVAudioSessionRouteChangeNotification, selector: #selector(AudioPlayer.audioSessionRouteChanged))
                    observe(AVAudioSessionMediaServicesWereLostNotification, selector: #selector(AudioPlayer.audioSessionMessedUp))
                    observe(AVAudioSessionMediaServicesWereResetNotification, selector: #selector(AudioPlayer.audioSessionMessedUp))
                #endif
                observe(AVPlayerItemDidPlayToEndTimeNotification, selector: #selector(AudioPlayer.playerItemDidEnd))
            }
        }
    }
    ...
    */
    public func resume() {
        ...
        if retryTimer == nil {
            ...
            let timer = NSTimer(timeInterval: retryTimeout, target: target, selector: #selector(ClosureContainer.callSelectorOnTarget), userInfo: nil, repeats: false)
            NSRunLoop.mainRunLoop().addTimer(timer, forMode: NSRunLoopCommonModes)
            retryTimer = timer
        }
    }
